...
const UPLOAD_SEGMENT = "portfolio-projects";
const MAX_BYTES = 12 * 1024 * 1024;

export function portfolioProjectUploadRoot(): string {
...
export const portfolioProjectArtifactUpload = multer({
  storage: multer.diskStorage({
    destination: (req, _file, cb) => {
      const projectId = req.params.id;
      const dir = path.join(portfolioProjectUploadRoot(), String(projectId));
      try {
        ensureDir(dir);
        cb(null, dir);
      } catch (e) {
...
export function toStoredPath(projectId: number, filename: string): string {
  return path.join(UPLOAD_SEGMENT, String(projectId), filename).replace(/\\/g, "/");
}
