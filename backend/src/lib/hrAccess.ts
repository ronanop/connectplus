...
/** Role names that grant HR module access (in addition to department/tags). */
export const HR_ACCESS_ROLE_NAMES = new Set(["HR_ADMIN", "HR"]);
...
function departmentIndicatesHr(department: string | null | undefined): boolean {
  ...
  // Default CRM seed uses "HR Department"; admins may use "Human Resources", etc.
  if (n === "hr department") {
    return true;
  }
...
export function tagsIndicateHr(tags: string[] | undefined | null): boolean {
  if (!tags?.length) {
    return false;
  }
  const hints = new Set([
...
export function userHasHrAccess(user: HrAccessUser): boolean {
