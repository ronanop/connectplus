    ...
    try {
      ...
      intervalRef.current = setInterval(async () => {
        try {
          ...
          /* skip frame: video not sized yet or transient tfjs error */
        }
      }, 800);
    } catch (e: unknown) {
        ...
        >
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-4 pb-8">
            {phase === "preview" && previewUrl ? (
              <>
                <img src={previewUrl} alt="Preview" className="max-h-[50vh] rounded-2xl object-contain" />
                <div className="flex gap-3">
                  <button
                    type="button"
                    className="min-h-[48px] rounded-xl border border-white/30 px-6 text-white"
                  ...
                  <button
                    type="button"
                    disabled={saving}
                    className="min-h-[48px] rounded-xl bg-emerald-600 px-6 font-medium text-white disabled:opacity-50"
              ...
              <>
                <div className="relative h-[300px] w-[300px] overflow-hidden rounded-full border-4 border-white/20 bg-neutral-900 sm:h-[360px] sm:w-[360px]">
                  <video
                    ref={videoRef}
                ...
                {phase === "idle" ? (
                  <button
                    ...
                    className="flex min-h-[48px] min-w-[48px] items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-neutral-900"
                  ...
                  <button
                    ...
                    disabled={!faceOk}
                    className="flex h-[72px] w-[72px] items-center justify-center rounded-full bg-emerald-500 text-white shadow-lg disabled:opacity-40"
