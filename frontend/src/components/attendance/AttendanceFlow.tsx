...
type Props = {
  ...
  onClose: () => void;
  onSuccess: () => void;
  /** checkOut runs the same geo + face path then calls POST /check-out */
...
export function AttendanceFlow({ open, onClose, onSuccess, mode = "checkIn" }: Props) {
    ...
    const runGeo = async () => {
      ...
      try {
        ...
        if (axios.isAxiosError(e)) {
          ...
          if (!e.response) {
            setGeoState("UNKNOWN");
            setGeoMsg("Cannot reach the server. Check your connection and try again.");
            return;
          }
        }
  ...
  useEffect(() => {
    ...
    /** AnimatePresence mode="wait" can delay mounting the <video>; wait until ref exists. */
  ...
  return (
    <div className="fixed inset-0 z-[250] flex flex-col bg-[var(--bg-base)] text-[var(--text-primary)]">
      ...
      {/** sync: step-2 <video> must mount immediately; "wait" delayed it until after step-1 exit, breaking getUserMedia */}
      <AnimatePresence mode="sync">
        {doneScreen === "success" ? (
          ...
          >
            ...
            <button
              type="button"
              className="mt-4 rounded-xl bg-[var(--accent-primary)] px-6 py-3 text-white"
              onClick={onClose}
            >
              Close
            </button>
          ...
          >
            ...
            <button type="button" className="mt-4 rounded-xl border px-6 py-3" onClick={onClose}>
              Close
            </button>
          </motion.div>
        ) : step === 1 ? (
          ...
          >
            ...
            {geoState === "OUTSIDE" && (
              <>
                ...
                <button type="button" className="rounded-xl border px-4 py-2" onClick={retryGeo}>
                  Retry
                </button>
            ...
            {(geoState === "DENIED" || geoState === "UNAVAILABLE" || geoState === "TIMEOUT" || geoState === "UNKNOWN") && (
              <>
                <p className="max-w-md text-center text-sm" aria-live="polite">
          ...
          <motion.div
            ...
            initial={{ x: 20, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            className="flex flex-1 flex-col items-center gap-4 overflow-y-auto p-4 pb-8"
          >
            ...
            <div className="relative mx-auto h-[300px] w-[300px] overflow-hidden rounded-full border-2 border-[var(--border)] bg-black sm:h-[360px] sm:w-[360px]">
              <video
                ref={videoRef}
            ...
            <button
              ...
              className="flex h-[72px] w-[72px] items-center justify-center rounded-full bg-emerald-600 text-sm font-semibold text-white shadow-lg disabled:opacity-40"
