...
/** Status values allowed on PATCH /status (completion uses other endpoints). */
...
export interface HierarchyTaskListItem {
  ...
  /** Set when assignmentMode is DEPARTMENT_HANDOFF — managers add ICs in this dept. */
  handoffTargetDepartment?: string | null;
  assignedBy: HierarchyPerson;
  /** All people assigned to the task (order preserved). */
  assignees: HierarchyPerson[];
  /** First assignee for backward compatibility; null if none. */
...
/** Duration + label for COMPLETION_APPROVED rows (handles legacy meta and JSON quirks). */
...
/** Human-readable span from milliseconds (for activity: time from task creation to completion). */
...
export const hierarchyTasksApi = {
  ...
  async list(params?: Record<string, string | undefined>): Promise<HierarchyTaskListItem[]> {
    const res = await api.get<ApiEnvelope<{ tasks: HierarchyTaskListItem[] }>>("/api/hierarchy-tasks", { params });
